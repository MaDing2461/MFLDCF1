...
class Restore2(nn.Module): # FLDCF
    def __init__(self,args, block=Bottleneck, layers=[1, 2, 2, 1], num_classes=2, aux=True):
        ...
        self.learned.load_state_dict(
                ...
                os.path.join('/media/lscsc/nas/mading/fakedetect/model', 'model_Potsdam_md_restore_dsm.pt'),  
                ),
                strict=True
            )
...
class Restorelocal(nn.Module):
    def __init__(self,args, block=Bottleneck, layers=[1, 2, 2, 1], num_classes=2, aux=True):
        self.inplanes = 64
        self.aux = aux	
        super(Restorelocal, self).__init__()

        self.learned = 	Restoretest(args)
        # self.learned.load_state_dict(
        #         torch.load(
        #             os.path.join('./model', 'model_lo.pt'),
        #         ),
        #         strict=True
        #     )
        self.learned.load_state_dict(
